...
class EXIF {
  ...
  getData(img: any, callback: Function) {
    ...
    if (isBase64(img)) {
      console.log('l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-isBase64-', img);
      file.src = img;
    } else if (img.path) {
      console.log('l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-isBase64-', img.path);
      file.src = img.path;
    } else if (isString(img)) {
      console.log('l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-isBase64-', img);
      file.src = img;
    } else {
      console.log('l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-false-');
      return false;
    }

    if (!imageHasData(file)) {
      console.log('l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-imageHasData-', file);
      getImageData(file, callback);
    } else {
      console.log('l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-callback-', file);
      if (callback) {
        callback.call(file);
      }
    }
  ...
  getTag(img: File, tag: string) {
    console.log(
      'l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-getTag-',
      img,
      '===',
      tag
    );
    console.log(
      'l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-imageHasData-',
      imageHasData(img)
    );
...
function getImageData(img: File, callback: Function) {
  ...
  if (img.src) {
    if (/^data\:/i.test(img.src)) {
      console.log(
        'l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-data.test(img.src)-',
        img.src
      );
      // Data URI
      ...
      console.log(
        'l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-blob.test(img.src)-',
        img.src
      );
      // Object URL
      ...
      console.log('l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-undefined-', img.src);
      ...
      console.log(
        'l-shrink--最新图片压缩结果imgshrikres--getImageOrientation-pathToBase64-',
        img.src
      );
  ...
  while (offset < length - 4) {
    if (getStringFromDB(dataView, offset, 4) == 'http') {
      ...
      xmpString =
        ...
        `xmlns:Iptc4xmpExt="http://iptc.org/std/Iptc4xmpExt/2008-02-29/" ${xmpString.slice(
          indexOfXmp
        )}`;
